...
export function expandLine(line: number[][], lineWidth: number) {
    ...
    return generateGeometry(leftPoints, rightPoints);
}
...
function generateGeometry(leftPoints: number[][], rightPoints: number[][]) {
    const positions = new Float32Array(leftPoints.length * 3 * 2);
    const indexes = new Int32Array((leftPoints.length - 1) * 2 * 3);
    for (let i = 0; i < leftPoints.length; i++) {
        positions[i * 6 + 0] = leftPoints[i][0];
        positions[i * 6 + 1] = leftPoints[i][1];
        positions[i * 6 + 2] = leftPoints[i][2];

        positions[i * 6 + 3] = rightPoints[i][0];
        positions[i * 6 + 4] = rightPoints[i][1];
        positions[i * 6 + 5] = rightPoints[i][2];

        if (i < leftPoints.length - 1) {
            indexes[i * 6 + 0] = i * 2 + 0;
            indexes[i * 6 + 1] = i * 2 + 1;
            indexes[i * 6 + 2] = (i + 1) * 2 + 0;
            indexes[i * 6 + 3] = (i + 1) * 2 + 0;
